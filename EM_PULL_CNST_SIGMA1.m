function [ efficiencyScore,allocationScore,reliabilityScore, maxSINR, numberOfServedUsers,iter] = EM_PULL_CNST_SIGMA1 (x,m,sigma_in,softmaxScale,maxUsersPerCluster,maxIter,sinr_threshold)
...
while iter < maxIter
    ...
    %%%%%%%%%%%entropy
    Probs = softmax((SINR)'.*softmaxScale)';
    entro = -log2(Probs).*Probs;
    entro(isnan(entro))=0;
    sum(sum(entro'))
    aaaa = sum(sum(log2(Probs)))
    %%%%%%%%%%%%
