function mobility_group = Generate_Members_Mobility_MY2(s_input,s_mobility)
...
for nodeIndex_leader = 1:s_input.NB_NODES
    ...
    for nodeIndex_group=1:s_mobility_group(nodeIndex_leader).NB_NODES_group
        ...
        while (time_in < s_input.SIMULATION_TIME)
            [leader_x_in,leader_y_in]= Walk_Member(leader_x_in,leader_y_in,time_in,s_input,s_mobility);
...
%time_final=s_mobility.VS_NODE(nodeIndex_leader).V_TIME(size(s_mobility_group(nodeIndex_leader).VS_NODE(nodeIndex_group).V_TIME,2)+1);
