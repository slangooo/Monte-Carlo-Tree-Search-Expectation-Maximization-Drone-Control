function  [reliabilitiesPerIterationMcts, numberOfStatesVisitedMcts,...
    energyEfficiencyPerIteration, numberOfServedUsersPerIteration,...
    EmIter,averageDronePower, spectralEfficiencyPerIteration] = runMcts (x, parameters)
...
sinrThresholds = -5:22;
averageDronePower = zeros(maxIter,1);

reliabilitiesPerIterationMcts = zeros(length(sinrThresholds),maxIter);
energyEfficiencyPerIteration = zeros(maxIter,1);
spectralEfficiencyPerIteration = zeros(maxIter,1);
numberOfServedUsersPerIteration = zeros(maxIter,1);

numberOfUsers = size(x,1);

root= mcts_node_rewards_FINAL(x,sigma,softmaxScale,maxUsersPerCluster,200,step,sinr_threshold,dronePower,numberOfUsers, desiredLinkReliability,0);

for iteration = 1:maxIter
    iterMcts = iteration
    root.update(x,root.merit);
    numberOfStatesVisitedMcts(iteration) = length(statesHistory_K_05_N_5);
    ...
    spectralEfficiencyPerIteration(iteration) = mean(log2(reliabilities_K_05_N_5 + 1));
    numberOfServedUsersPerIteration(iteration) = maxAllocationScore_K_05_N_5;
    averageDronePower(iteration) = sum(maxSigma)/m_initial;
    else
